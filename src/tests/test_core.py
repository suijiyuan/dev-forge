    ...
    def test_builds_expected_zip(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with zipfile.ZipFile(output) as archive:
                ...
                self.assertIn("function Get-CodeInstallations", install_script)
                self.assertIn("function Find-CodeCommand", install_script)
                self.assertIn("function Get-CodeInstallationInfo", install_script)
                self.assertIn("Kind = 'archive'", install_script)
                self.assertIn("Kind = 'user'", install_script)
                self.assertIn("Kind = 'system'", install_script)
